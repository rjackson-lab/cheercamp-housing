...
async function runPlacement(rawBuf, blankBuf, referenceBuf, options = {}) {
  ...
  const { assignments, warnings } = core.buildPlacements(beds, halls, options);
  ...
  const headerMods = [];
  const seenHeaderMods = new Set();
  for (const hall of halls) {
    const colNum = hall && hall.cols['Group/School'];
    const headerRow = hall && hall.cols._headerRow;
    if (!colNum || !headerRow) continue;
    const key = `${hall.name}|${colNum}|${headerRow}`;
    if (seenHeaderMods.has(key)) continue;
    seenHeaderMods.add(key);
    headerMods.push({ sheet: hall.name, row: headerRow, col: colLetter(colNum), value: 'Group/School' });
  }
  const mods = headerMods.concat(enrichedAssignments.map(a => {
...
const OUTPUT_COLUMN_ALIASES = {
  ...
  entryFirst: ['entry name first'],
  entryLast: ['entry name last'],
...
async function finalizeWithNames(rawBuf, placeholderBuf, assignments) {
  ...
  for (const a of assignments || []) {
    ...
    setCell(ws, a.row, headers.first ?? headers.entryFirst, person.first);
    setCell(ws, a.row, headers.last ?? headers.entryLast, person.last);
...
/**
 * Recompute compliance + regenerate the placeholder workbook after assignments
 * have been edited (manually or by the assistant). Keeps blockers/status and the
 * downloadable placeholder in sync with the current assignment labels/genders.
 * Pure recompute — invents no new placements; only re-checks the given assignments.
 */
async function recomputeAfterEdit(rawBuf, blankBuf, referenceBuf, assignments) {
  const rawWB = XLSX.read(rawBuf, { type: 'buffer' });
  const blankWB = XLSX.read(blankBuf, { type: 'buffer', cellStyles: true });
  const refWB = referenceBuf ? XLSX.read(referenceBuf, { type: 'buffer', cellStyles: true }) : blankWB;
  ...
  const mods = headerMods.concat((assignments || []).map(a => {
    const hall = halls.find(h => h.name === a.sheet);
    const colNum = hall && hall.cols['Group/School'];
    return colNum ? { sheet: a.sheet, row: a.row, col: colLetter(colNum), value: a.label } : null;
  }).filter(Boolean));
  ...
  return {
    ...
    beds: beds.length,
    placed: (assignments || []).length,
    outputBuffer: Buffer.from(outputAB)
  };
}
...
module.exports = { runPlacement, computeStatus, finalizeWithNames, recomputeAfterEdit };
