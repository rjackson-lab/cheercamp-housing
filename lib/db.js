...
if (adminCount === 0) {
  ...
  const configuredEmail = process.env.PARKER_EMAIL;
  const configuredPw = process.env.PARKER_PW;
  if (isProduction && (!configuredEmail || !configuredPw)) {
    throw new Error('PARKER_EMAIL and PARKER_PW must be set before first production boot.');
  }
  const defaultEmail = (configuredEmail || 'pjackson@varsity.com').toLowerCase();
  const defaultPw = configuredPw || 'Varsity2026';
  if (defaultPw.length < 8) {
    throw new Error('PARKER_PW must be at least 8 characters.');
  }
