...
const COLUMN_ALIASES = {
  RoomID: ['roomid', 'room id', 'room', 'room number', 'room #', 'room no', 'room no.', 'bedroom', 'bed', 'bed #', 'bed number', 'space', 'space id', 'assignment space'],
...
function roomLike(value) {
  ...
  return ROOM_ID_RE.test(s) || /^[A-Za-z]?\d{1,4}[A-Za-z0-9-]*$/i.test(s);
}
...
function inferTemplateColumns(sheet, cols) {
  ...
  if (!groupCol) {
    const candidates = Object.entries(stats)
      .filter(([c]) => Number(c) !== roomCol && Number(c) !== floorCol)
      .sort((a, b) => b[1].empty - a[1].empty);
    groupCol = Number(candidates[0]?.[0] || 0) || (range.e.c + 2);
  }

  return { roomCol, floorCol, groupCol, headerRow };
}
...
function parseTemplate(blankWB, referenceWB) {
  ...
  for (const sheetName of blankWB.SheetNames) {
    ...
    for (let row1 = inferred.headerRow + 1; row1 <= range.e.r + 1; row1++) {
      ...
      if (!roomLike(ridStr)) continue;
      const floorCell = colFloor ? cellAt(ws, row1, colFloor) : null;
      const floor = normalizeFloorValue(floorCell ? floorCell.v : null, ridStr);
