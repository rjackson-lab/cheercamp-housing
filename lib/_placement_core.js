...
const ROOM_ID_RE = /^[A-Za-z]{2,4}\s*\d+\s*[A-Z]*\s*$|^[A-Za-z]{2,4}\d+\s*RA$/i;
...
async function surgicalWrite(inputArrayBuffer, modifications) {
  ...
  for (const [sheetName, cellMods] of Object.entries(bySheet)) {
    ...
    for (const [cellRef, value] of cellMods) {
      xml = patchCellXml(xml, cellRef, value);
    }
    zip.file(file, xml);
  }
...
function detectColumns(sheet) {
  ...
  const r = HEADER_ROW - 1; // 0-indexed
  for (let c = range.s.c; c <= range.e.c; c++) {
    const cell = sheet[XLSX.utils.encode_cell({ r, c })];
    if (cell && cell.v != null) cols[String(cell.v).trim()] = c + 1;
  }
  return cols;
}
