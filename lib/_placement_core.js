...
const COLUMN_ALIASES = {
  RoomID: ['roomid', 'room id', 'room', 'room number', 'room #', 'room no', 'room no.', 'bedroom', 'space'],
  Floor: ['floor', 'level'],
  'Group/School': ['group/school', 'group school', 'school/group', 'school', 'group', 'account', 'team', 'organization'],
...
function ingestRaw(workbook) {
  ...
  for (let i = 0; i < rows.length; i++) {
    const r = rows[i];
    const early = String(r['Early Arrival/Late Departure'] || '').trim().toUpperCase() === 'X';
    if (r['Primary Guest Role']) {
      beds.push({
        account:  String(r['Account'] || '').trim(),
        first:    cleanStr(r['Primary Guest First Name']),
        last:     cleanStr(r['Primary Guest Last Name']),
        role:     String(r['Primary Guest Role']),
        category: ROLE_CATEGORIES[r['Primary Guest Role']] || 'Other',
        gender:   cleanStr(r['Primary Guest Gender']),
        early,
        raw_row:  i + 2,
      });
    }
    if (Number(r['Occupancy']) === 2 && r['Guest 2 Role']) {
      beds.push({
        account:  String(r['Account'] || '').trim(),
        first:    cleanStr(r['Guest 2 First']),
        last:     cleanStr(r['Guest 2 Last']),
        role:     String(r['Guest 2 Role']),
        category: ROLE_CATEGORIES[r['Guest 2 Role']] || 'Other',
        gender:   cleanStr(r['Guest 2 Gender']),
        early,
        raw_row:  i + 2,
      });
    }
...
function parseTemplate(blankWB, referenceWB) {
  ...
  for (const sheetName of blankWB.SheetNames) {
    ...
    const colRoomId = cols['RoomID'];
    const colFloor = cols['Floor'];
    const colGroup = cols['Group/School'];
    const colNotes = cols['Notes'];
    if (!colRoomId || !colFloor || !colGroup) continue;
    const refCols = ws_ref ? detectColumns(ws_ref) : {};
    const refColGroup = refCols['Group/School'];

    const rooms = [];
    const range = XLSX.utils.decode_range(ws['!ref']);
    for (let row1 = (cols._headerRow || HEADER_ROW) + 1; row1 <= range.e.r + 1; row1++) {
      const ridCell = cellAt(ws, row1, colRoomId);
      const rid = ridCell ? ridCell.v : null;
      if (rid == null) continue;
      const ridStr = String(rid).trim();
      if (!ridStr || !/\d/.test(ridStr) || !ROOM_ID_RE.test(ridStr)) continue;
      const floorCell = cellAt(ws, row1, colFloor);
      const floor = floorCell ? floorCell.v : null;
      ...
      rooms.push({
        sheet: sheetName, row: row1, room_id: ridStr,
        floor, is_ra: !!isRA, is_excluded: false,
        ref_label: refLabel, suite_id: suiteId,
      });
    }

    const isStaff = sheetName.includes('Staff');
...
/* ============================================================
   ...
   ============================================================ */
function buildPlacements(beds, halls) {
  const assignments = [];
  const warnings = [];
  ...
  const fillPodsGenderAware = (podList, pools) => {
    ...
    for (let i = 0; i < podList.length; i++) {
      ...
      for (const bed of pod.beds) {
        ...
        assignments.push({ sheet: bed.sheet, row: bed.row, label: pools[gender].shift(), gender });
      }
    }
    return consumed;
  };

  // After placing a team, consume the pods used + 1 buffer pod when feasible
  const advanceFloor = (fk, consumed) => {
    const remaining = inv[fk].length - consumed;
    const buffer = remaining > 0 ? 1 : 0;
    inv[fk] = inv[fk].slice(consumed + buffer);
  };
    ...
    for (const [acct, by_g] of Object.entries(beds_by_acct)) {
      for (const [g, n] of Object.entries(by_g)) {
        ...
        while (placedN < n && pool.length > 0) {
          ...
          assignments.push({ sheet: bed.sheet, row: bed.row, label: acct, gender: g });
          placedN++;
        }
